...
export interface Catalogue {
  find(name: string): Promise<Game | null>;
}
export interface PlayWriter {
  save(players: string[]): Promise<void>;
}
...
export async function countPlays(reader: PlayReader) {
  return (await reader.all()).length;
}
