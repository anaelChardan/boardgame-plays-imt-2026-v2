...
function run(command, args, cwd=root, capture=false) {
  ...
  if(r.status!==0) throw new Error(capture ? (r.stderr || `${command} a échoué`) : `${command} a échoué (${r.status})`);
  return r.stdout?.trim() || '';
}
function step(value) {
  const s=steps.find(s=>s.step===String(value).padStart(2,'0') || s.name===value);
  if(!s) throw new Error('Étape inconnue. Utiliser : npm run course -- list');
  return s;
}
function show(s) {
...
function prepare(s) {
 ...
 const fingerprint=sha+':'+process.version+':'+createHash('sha256').update(readFileSync(resolve(dir,'package-lock.json'))).digest('hex');
 if(!existsSync(marker) || readFileSync(marker,'utf8')!==fingerprint) {
  run(npm,['ci','--no-audit','--no-fund'],dir);
  run(npm,['run','build'],dir);
  writeFileSync(marker,fingerprint);
 }
 show(s);console.log(`Dossier : ${dir}
...
try {
 if(action==='warmup') steps.forEach(prepare);
  ...
  let s=step(id);
  if(action==='next') { const next=steps[steps.indexOf(s)+1]; if(!next) throw new Error('Dernière étape atteinte.'); s=next; }
  const dir=prepare(s);
