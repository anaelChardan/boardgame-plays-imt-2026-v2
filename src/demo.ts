...
console.log(
  '09 · Nouvelle règle : noms uniques après suppression des espaces et normalisation de la casse',
);
try {
  ...
  const output: string[] = [];
  const status = await runCli(['Azul', ...request.players], play, (line) =>
    output.push(line),
  );
  ...
  console.log(
    'Les deux refus laissent le stockage vide. Une règle métier protège les deux entrées.',
  );
  const accepted = await http.inject({
    method: 'POST',
    url: '/plays',
    payload: { ...request, players: ['Alice', 'Bob'] },
  });
