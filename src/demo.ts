...
console.log(
  '05 · BGG : remplacer un adaptateur secondaire sans changer le cas d’usage',
);
console.log(
  'Vrai adaptateur XML, réponses HTTP contrôlées : aucun réseau ni jeton réel.',
);
const searchXml =
  '<items><item id="230802"><name value="Azul"/></item></items>';
const detailsXml =
...
const controlledFetch: typeof fetch = async (input) => {
  ...
  return new Response(
    url.pathname.endsWith('/thing')
      ? detailsXml
      : url.searchParams.get('query') === 'Azul'
        ? searchXml
        : '<items total="0"/>',
  );
};
...
const unavailable = buildBggCatalogue(
  'demo-offline',
  async () => new Response('', { status: 401 }),
);
await assert.rejects(
  () => unavailable.getBoardgameByName('Azul'),
  CatalogUnavailable,
);
console.log(
  'HTTP 401 → CatalogUnavailable, panne technique distincte de l’absence.',
);
