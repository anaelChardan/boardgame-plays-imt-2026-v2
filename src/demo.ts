...
console.log('08 · HTTP et CLI : deux adaptateurs primaires, le même cas d’usage');
console.log('Démonstration dans un processus, avec une mémoire partagée et HTTP interne.');
try {
  ...
  const output: string[] = [];
  const status = await runCli(['Azul', 'Alice', 'Bob'], play, line => output.push(line));
  ...
  const refused = await http.inject({ method: 'POST', url: '/plays', payload: { ...request, players: ['Alice'] } });
  assert.equal(refused.statusCode, 422);
  assert.equal(await runCli(['Azul', 'Alice'], play, console.log), 1);
  assert.equal((await store.all()).length, 2);
  console.log('Un joueur → HTTP 422 et CLI code 1 ; toujours 2 parties sauvegardées.');
