...
const details = z.object({
  ...
  name: z.union([
    ...
    z.array(z.object({ type: z.string(), value: z.string() })),
  ]),
  minplayers: z.object({ value: z.coerce.number().int().positive() }),
  maxplayers: z.object({ value: z.coerce.number().int().positive() }),
});
export function buildBggCatalogue(
  token: string,
  fetcher: typeof fetch = fetch,
): BoardgameInventory {
  if (!token.trim()) {
    throw new Error('BGG_API_TOKEN requis pour le catalogue bgg');
  }
    ...
    async getBoardgameByName(name) {
      try {
        const result = await get(
          `search?type=boardgame&exact=1&query=${encodeURIComponent(name)}`,
        );
        const candidates = asArray(result?.item).map((item) =>
          searchItem.parse(item),
        );
        const found = candidates.find(
          (item) => item.name.value.toLowerCase() === name.toLowerCase(),
        );
        if (!found) {
          return null;
        }
        ...
        return {
          name: primary.value,
          bggId: game.id,
          minNumberOfPlayers: game.minplayers.value,
          maxNumberOfPlayers: game.maxplayers.value,
        };
      } catch {
        throw new CatalogUnavailable();
      }
