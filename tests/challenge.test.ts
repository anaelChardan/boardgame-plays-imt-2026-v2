...
it.each([['Alice','Alice'],['Alice',' alice ']])('refuse les doublons %j dans HTTP et CLI sans enregistrer',async (first,second)=>{
 const store=buildMemoryStore();const play=buildPlayAGame(fixtureCatalogue,store);const app=buildHttp(play,store);
 try {
  const response=await app.inject({method:'POST',url:'/plays',payload:{boardgameName:'Azul',players:[first,second]}});
