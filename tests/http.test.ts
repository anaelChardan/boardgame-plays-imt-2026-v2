...
it.each([
  ...
  try { expect((await app.inject({method:'POST',url:'/plays',payload})).statusCode).toBe(status); }
  finally { await app.close(); }
});
it('distingue panne du catalogue et jeu inconnu', async () => {
  const app = buildHttp(async () => {throw new CatalogUnavailable();});
  try { expect((await app.inject({method:'POST',url:'/plays',payload:{boardgameName:'Azul',players:['A','B']}})).statusCode).toBe(503); }
  finally { await app.close(); }
});
