...
it('enregistre une seule partie valide et aucune partie invalide',async()=>{
 ...
 await expect(play({boardgameName:'Inconnu',players:['Alice','Bob']})).rejects.toThrow();
...
it('protège les données des mutations externes',async()=>{
 ...
 const read=await store.all(); read[0]!.players.push('Mallory');
 expect((await store.all())[0]?.players).toEqual(['Alice','Bob']);
});
it('ne répond pas créé si le stockage échoue',async()=>{
 const play=buildPlayAGame(fixtureCatalogue,{async save(){throw new Error('disk failure');}});
 const app=buildHttp(play);
 try {expect((await app.inject({method:'POST',url:'/plays',payload:{boardgameName:'Azul',players:['Alice','Bob']}})).statusCode).toBe(500);}
