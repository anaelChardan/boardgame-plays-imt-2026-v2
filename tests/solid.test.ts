...
import { brokenCatalogue, emptyCatalogue, makeValidator, countPlays } from '../examples/solid/principles.js';
...
it('remplace un catalogue sans modifier la validation', async () => {
  const validate = makeValidator({async find() {return {name:'Brass',min:2,max:4};}});
...
it('ne demande que la lecture', async () => {
  expect(await countPlays({async all() {return [['Alice','Bob']];}})).toBe(1);
});
